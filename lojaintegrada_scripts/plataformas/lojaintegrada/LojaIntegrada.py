...
from plataformas.helpers import FileHandler, to_date
...
class LojaIntegrada(LojaIntegradaAPI):
    ...
    def lista_atualizacoes_por_data(self, data: date, limit: int = 20):
        ...
        while True:
            ...
            if (
                to_date(first_date) < data
                or to_date(last_date) < data
                or not meta.get("previous", False)
            ):
                break
    ...
    def lista_atualizacoes_por_datas(self, datas: list[date], limit: int = 20):
        ...
        if dados:
            logger.debug("retornando dados importados")
            return dados
