...
class MercadoPago(PagamentoABC):
    ...
    def _execute_get(self, resource, params={}):
        ...
        response = requests.get(
            self._base_url + resource, params=_params, headers=self.headers, timeout=50
        )
    ...
    def adapt_response(self, response):
        taxas = str(
            sum(
                [
                    float(fee["amount"])
                    for fee in filter(
                        lambda fee: fee.get("fee_payer", "") == "collector",
                        response.get("fee_details", []),
                    )
                ]
            )
        )
        ...
        return {
            "total_liquido": response.get("transaction_details", {}).get(
                "net_received_amount", ""
            ),
