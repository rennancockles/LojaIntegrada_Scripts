...
class PagSeguro(PagamentoABC):
    ...
    def __init__(self, email: str, token: str):
        self.base_params = {"email": email, "token": token}
        self._testa_sessao()
    ...
    def _execute_post(self, resource, params={}):
        ...
        response = requests.post(self._base_url + resource, params=_params, timeout=50)
    ...
    def _execute_get(self, resource, params={}):
        ...
        response = requests.get(self._base_url + resource, params=_params, timeout=50)
    ...
    def adapt_response(self, response):
        ...
        taxas = str(
            sum(
                [
                    float(value)
                    for value in detalhe_pagamento.get("creditorFees", {}).values()
                ]
            )
        )
        liberacao_pagamento = detalhe_pagamento.get("escrowEndDate", "")
        if liberacao_pagamento:
            liberacao_pagamento = datetime.strptime(
                liberacao_pagamento.split("T")[0], "%Y-%m-%d"
            ).strftime("%d/%m/%Y")
