...
from helpers import month_names, to_money
...
class Declaracao:
    ...
    def pedido_mapper(self, pedido):
        ...
        return {
            ...
            "ord_value": to_money(
                sum([line["itm_price"] for line in itm_lines]), currency="R$"
            ),
    ...
    def gera_declaracao(self, dados):
        declaracao_path = path.join(CWD, "../templates/declaracao.html")
        with open(declaracao_path, "r", encoding="utf-8") as f:
            html = f.read()

        declaracao_item_path = path.join(CWD, "../templates/declaracao_item.html")
        with open(declaracao_item_path, "r", encoding="utf-8") as f:
            html_item = f.read()

        print_css_path = path.join(CWD, "../templates/print.css")
        with open(print_css_path, "r", encoding="utf-8") as f:
            css = f.read()

        itm_lines = [
            html_item.replace("@itm_count", str(line["itm_count"]))
        ...
        declaracao = (
            html.replace("@css", f"<style>{css}</style>")
        ...
        pedido_declaracao_path = path.join(
            Path.home(), "Desktop", f'{dados["order_number"]}_declaracao.html'
        )
        with open(pedido_declaracao_path, "w", encoding="utf-8") as f:
            f.write(declaracao)
